...
export class Hotkey {
  private formattedHotkey: string[] | undefined;
  ...
  static symbolize(combo: string): string {
    ...
    for (let i = 0; i < comboSplit.length; i++) {
      ...
      if (comboSplit[i] === 'mod') {
        if (typeof window !== 'undefined' && window.navigator && window.navigator.platform.indexOf('Mac') >= 0) {
  ...
  get formatted(): string[] {
    if (!this.formattedHotkey) {
      const sequence: string[] = [...(this.combo as string[])];
